...
function CreateFoodJS() {
  ...
  useEffect(() => {
    function getItems() {
      const token = localStorage.getItem("authToken");
      axios
        .get("http://localhost:8070/resInventory/", {
          headers: {
            Authorization: `Bearer ${token}`, // Send token in Authorization header
          },
        })
    ...
    >
      ...
      <div className="data">
        <div className="backgroundBorder">
          <Form className="boldfont" style={{ width: "100%" }}>
            <Form.Group>
              <div className="btns">
                ...
                >
                  <option defaultValue={"Special Dish"}>
          ...
          >
            <button className="add_new" onClick={() => navigate(-1)}>
              Go Back
            </button>

            <button className="add_new" onClick={() => navigate("ViewDish")}>
              View Dishes
            </button>
          ...
          >
            <tbody>
              <tr>
                ...
                <td colSpan={2}>Total Cost for the Dish's Ingredients :</td>
                <td>
