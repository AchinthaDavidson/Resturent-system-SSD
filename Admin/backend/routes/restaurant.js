...
router.route("/update/:id").put(async (req, res) => {
  ...
  await Restaurant.updateOne({ Item_Id: Id }, { $set: updatebar })
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .send({ status: "bar inventory update failed", error: err });
    });
});
...
router.route("/update1/:id").post(async (req, res) => {
  ...
  Restaurant.find({ Item_Id: req.params.id })
  ...
  function update2(qty, cost3) {
    ...
    Restaurant.updateOne(
      { Item_Id: Id },
      { $set: { Quantity: Quantity3, Total_Cost: Total_Cost3 } }
    )
...
router.route("/updateqty").post(async (req, res) => {
  ...
  for (let i = 0; i < Id.length; i++) {
    if (!isValidObjectId(Id[i].Iid)) {
      return res
        .status(400)
        .json({ error: `Invalid ID format for Iid: ${Id[i].Iid}` });
    }
  }

  for (let i = 0; i < Id.length; i++) {
    food
      .findById({ _id: { $eq: Id[i].Iid } })
      .then((food) => {
        if (!food) {
          return res
            .status(404)
            .json({ error: `Food item not found for Iid: ${Id[i].Iid}` });
        }
    ...
    for (let i = 0; i < Ingridients.length; i++) {
      if (!isValidObjectId(Ingridients[i].id)) {
        return res.status(400).json({
          error: `Invalid ID format for ingredient: ${Ingridients[i].id}`,
        });
      }

      Restaurant.findById(Ingridients[i].id)
        .then((Restaurant) => {
          if (!Restaurant) {
            return res.status(404).json({
              error: `Restaurant item not found for ID: ${Ingridients[i].id}`,
            });
          }
...
router.route("/sum").get((req, res) => {
  Restaurant.aggregate([
    { $group: { _id: null, price: { $sum: "$Total_Cost" } } },
  ])
    .then((Restaurant) => {
      res.json(Restaurant);
    })
